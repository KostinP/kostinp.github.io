...
// Anchor id shared by a Projects-section card and any Work entry that
// links to it, so "related project" links can jump within the page.
export const projectAnchor = (slug) => `project-${slug}`;

export async function getWorks(locale, focus) {
  const [worksAll, projectsAll] = await Promise.all([
    reader.collections.works.all(),
    reader.collections.projects.all(),
  ]);
  const projectsBySlug = new Map(projectsAll.map(({ slug, entry }) => [slug, entry]));
  const inFocus = worksAll.map(({ entry }) => entry).filter((entry) => entry.focuses.includes(focus));
  const withBody = await Promise.all(
    inFocus.map(async (entry) => ({
      ...
      skills: entry.skills.map((skill) => loc(skill, locale)),
      // Only link projects that are actually shown for the current focus —
      // otherwise the anchor would point at a card that isn't on the page.
      relatedProjects: entry.projects
        .map((slug) => ({ slug, project: projectsBySlug.get(slug) }))
        .filter(({ project }) => project && project.focuses.includes(focus))
        .map(({ slug, project }) => ({ label: loc(project.name, locale), href: `#${projectAnchor(slug)}` })),
...
export async function getProjects(locale, focus) {
  ...
  const inFocus = all.filter(({ entry }) => entry.focuses.includes(focus));
  const withBody = await Promise.all(
    inFocus.map(async ({ slug, entry }) => ({
      slug,
      anchor: projectAnchor(slug),
